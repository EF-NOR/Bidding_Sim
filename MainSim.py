...
def reinject_hydro_to_supply(
    ...
    """
    Add reinjected hydro back into the supply curve at a given price.
 
    The reinjected hydro is bid at the scenario clearing price of the
    *source* hour (the high-wind hour where it was displaced). This is
    a reasonable approximation: the hydro producer knows their water is
    worth at least what it earned in the high-wind hour.
 
    In practice you can also bid it at the baseline clearing price or
    at a fixed price — adjust bid_price as needed.
    """
    if reinject_mw <= 0:
        return supply_cum
 
    s = supply_cum.copy().astype(float)
    mask = s.index >= bid_price
    s.loc[mask] += reinject_mw
 
    if bid_price not in s.index:
        vol_below = float(s[s.index < bid_price].iloc[-1]) if (s.index < bid_price).any() else 0.0
        s.loc[bid_price] = vol_below + reinject_mw
        s = s.sort_index()
...
def main():
    ...
    print(f"Actual wind loaded:    {len(wind_actual)} hours")
    print(f"Simulated wind loaded: {len(wind_sim)} hours")
    ...
    price_cap   = float(np.percentile(wind_price_series[wind_price_series < 4000], 90))
    ...
    print(f"  Wind price weight cap (90th pct, excl 4000): {price_cap:.1f} EUR/MWh")
